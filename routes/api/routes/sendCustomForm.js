...
transporter.verify((err, succes) => {
  err ? console.log("validatie mislukt: " + err) : console.log(`Send custom form: ${succes}`);
});
...
sendEmailRouter.post("/", async (req, res, next) => {
  const messageOwner = {
    ...
    subject: "Aanmelding schoolkorfbal 2026",
    html: `
  ...
  const messageSender = {
    ...
    html: `
		<h1>Geluk!</h1>
		<p>We hebben je bericht in goede orde ontvangen. Na 26 januari krijg je per mail het programma toegestuurd.</p>
    <p>Dit is wat je ons gestuurd hebt:</p>
    <table border="0">
      <tr>
        <td style="padding: 10px 15px; border: 1px solid black;">Geslacht</td>
        <td style="padding: 10px 15px; border: 1px solid black;">${req.body.gender}</td>
    ...
    <p> Met vriendelijke groet,<br>AW.DTV</p>
		`,
  };
  Promise.all([transporter.sendMail(messageOwner), transporter.sendMail(messageSender)])
